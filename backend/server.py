...
class CodeInspectorHandler(SimpleHTTPRequestHandler):
    ...
    def _handle_excel_report_flush(self, request_id: str):
        ...
        if not isinstance(body, dict):
            raise ValueError("JSON body must be an object")
        session_id = body.get("session_id") or body.get("output_dir") or None
        wait = body.get("wait", True)
        timeout_sec = body.get("timeout_sec", None)
        if session_id is not None and not isinstance(session_id, str):
            raise ValueError("session_id/output_dir must be a string when provided")
        if not isinstance(wait, bool):
            raise ValueError("wait must be a boolean")
        if timeout_sec is not None and not isinstance(timeout_sec, (int, float)):
            raise ValueError("timeout_sec must be numeric when provided")

        logger.info("Excel report flush start id=%s session=%s", request_id, session_id or "<last>")
