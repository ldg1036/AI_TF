...
<results version="2">
  <errors>
    ...
    def test_goldentime_reference_mismatch_threshold(self):
        ref_candidates = glob.glob(
            os.path.join(self.data_dir, "(코드리뷰결과서-Server) 코드 리뷰 결과서-HMI_천안 HVAC 유지보수_GoldenTime_3차_*.xlsx")
        )
        if not ref_candidates:
            self.skipTest("GoldenTime reference workbook not found")

        if not os.path.exists(os.path.join(self.data_dir, "GoldenTime.ctl")):
            self.skipTest("GoldenTime.ctl not found")
        ...
        output_dir = os.path.normpath(result.get("output_dir", ""))
        generated_files = sorted(glob.glob(os.path.join(output_dir, "CodeReview_Submission_GoldenTime_*.xlsx")))
        self.assertTrue(generated_files, "Generated GoldenTime workbook not found")

        compare_result = compare_reports(ref_candidates[0], generated_files[-1])
        self.assertLessEqual(compare_result.get("mismatch_count", 999), 5)
